export default class BuildPDF {
  ...
  getPDFConfig() {
    ...
    return {
      apiUrl: runtime.apiUrl || "https://pdf.store.tailormade.uk/generate-pdf",
      stylesheetUrl:
        runtime.stylesheetUrl ||
        "https://pdf.store.tailormade.uk/static/pdf-styles.css",
      fontCssUrl:
        runtime.fontCssUrl ||
        "https://fast.fonts.net/cssapi/939a4cc7-4305-49d3-9eb7-d6746fdc66d3.css",
    };
  }
  ...
  getPdfData(productPage, sku = "") {
    // Get product title
    const productTitle =
      productPage.querySelector(".status-title")?.innerText.trim() || "";

    // Get product price
    const productPrice =
      productPage.querySelector(".status-price")?.innerText.trim() || "";

    // Add configured price text to price value if it exists
    const configuredPrice = productPrice
      ? `CONFIGURED PRICE: ${productPrice}`
      : productPrice;
    ...
    // Get product image
    const productImage =
      productPage.querySelector(".status-image img")?.src || "";

    // Get current spec text
    const specTextBlock =
      productPage.querySelector(".status-specifications .d-block") || "";
    ...
    const swatches = Array.from(
      productPage.querySelectorAll(".status-layer-img img"),
    ).filter((swatch) => {
      ...
      const hiddenParent = layerEl.closest(
        '.d-none, [hidden], [style*="display:none"]',
      );
    ...
    const swatchData = swatches.map((swatch, index) => {
      ...
      const swatchLabel =
        layerEl?.querySelector(".status-layer-title")?.innerText.trim() ||
        fallbackLabel;
      const swatchValue =
        layerEl?.querySelector(".status-layer-colour")?.innerText.trim() || "";
    ...
    if (!productTitle)
      console.log("[BuildPDF] Missing product title: .product-title");
    if (!qrCode) console.log("[BuildPDF] Missing QR code: .qrcode");
    if (!configuredPrice)
      console.log("[BuildPDF] Missing product price: .status-price");
    if (!productImage)
      console.log("[BuildPDF] Missing product image: .status-image img");
    if (!specText)
      console.log(
        "[BuildPDF] Missing spec text: .status-specifications .d-block",
      );
    if (!swatchData.length)
      console.log("[BuildPDF] No swatches found: .status-layer-img img");
  ...
  findQrSource(root) {
    const selector =
      ".status-qrcode, .qrcode, #qrcode, .status-qr, .status-qr img";
