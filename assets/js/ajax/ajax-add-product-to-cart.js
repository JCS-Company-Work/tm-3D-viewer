export default class ProductAddToCart {
  ...
  initAddToCartButton() {
    ...
    const state = {
      ...
      defaultButtonText:
        addToCartBtn && addToCartBtn.textContent.trim()
    ...
    addToCartBtn.addEventListener("click", async (e) => {
      ...
      const currentUrl =
        document.querySelector("form.cart")?.action || window.location.href;
      ...
      // Get model size
      const model =
        document.querySelector(".obj-model select option:checked")?.value || "";

      // Get metal edge veneer value if set (optional field)
      const veneer =
        document.querySelector(
          '.obj-metal-edge-veneer input[type="radio"]:checked',
        )?.value || "";

      // Get quantity
      const quantity =
        document.querySelector('input.qty[name="quantity"]')?.value || 1;

      // Get current configuration image from DOM
      const imageUrl = this.getBasketImageUrl() || "";

      // Get configured total
      const configuredTotal = parseFloat(
        document
          .getElementById("configured-total")
          ?.value.replace(/[^0-9.]/g, "") || 0,
      );

      if (!productId || isNaN(productId)) {
        this.handleError(
          new Error("Invalid or missing product ID"),
          addToCartBtn,
          state,
        );
        return;
      ...
      // Use a standard URL-encoded request because this payload contains only scalar values.
